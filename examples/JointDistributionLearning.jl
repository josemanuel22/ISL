...
epsi = 0.2
realModel(x, ϵ) =  ϵ < 0.2 ? model1(x) : model2(x)

#Learning with custom loss
μ = 0; stddev = 1
η = 0.1; num_epochs = 100; n_samples = 3000; K = 20
...
@showprogress for epoch in 1:num_epochs
    loss, grads = Flux.withgradient(model) do m
        ...
        mean_approx = mean(ŷ)
        std_approx = std(ŷ)
        y = rand(Normal(epsi*b₁+ (1-epsi)*b₂, sqrt(epsi^2*m₁^2 + (1-epsi)^2*m₂^2)), 100)
